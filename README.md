...
`lib/seo.ts` builds each page's metadata: a self-referencing canonical, Open
Graph and Twitter card tags, and the share image generated by `app/og/route.tsx`.
Every page calls `pageMetadata()`; `robots` defaults to index/follow in the root
layout.

`lib/schema.ts` builds the JSON-LD, rendered through `components/seo/JsonLd.tsx`.
Organization and WebSite go in the root layout; the sub-page template adds
Service, BreadcrumbList and FAQPage; the contact page adds both offices as
ProfessionalService.

The share image and canonical URLs are absolute and built from `SITE_URL`, so a
staging build that should point at itself needs `SITE_URL` set accordingly.
